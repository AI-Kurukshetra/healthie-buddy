...
- All pages must use a centered container layout (max-width 1200px).
- Forms must use shadcn/ui components.
- Inputs must have labels and helper text.
- Use Card layout for forms and dashboards.
- Add spacing scale using Tailwind (space-y-4 / space-y-6).
- Buttons must use primary/secondary variants.
- Use consistent typography hierarchy:
  - h1 page title
  - h2 section title
  - muted description text
- Coordinator must invoke $frontend-design for all UI tasks.
- Coordinator must never generate UI directly.
...
- Role selector must be a dropdown
...
All code must be written inside the repository root.

Never scaffold a new Next.js project inside a subdirectory
